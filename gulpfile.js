...
gulp.task("generate-options-readme-section", (done) => {
    const packageJSON = require("./package.json");
    const packageNLS = require("./package.nls.json");
    const configurationProperties =
        packageJSON.contributes.configuration.properties;
    const uniqueOptions = Object.keys(configurationProperties);
    const readmeJSON = {};
    for (const option of uniqueOptions) {
        const defaultValue = configurationProperties[option].default;
        readmeJSON[option] = defaultValue;
    }

    let optionsReadmeSection =
        "## Options\n\n" +
        ...
        "```javascript" +
        "\n" +
        JSON.stringify(readmeJSON, null, 2) +
        "\n" +
        "```" +
        "\n" +
        "### Options Reference\n\n" +
    ...
    // Making a table for options reference
    for(const option of Object.keys(configurationProperties)) {
        if (option === "mssql.shortcuts") {
            continue;
        }
        const description = configurationProperties[option].description;
        let descriptionText = "";
        if (description) {
            // Removing trailing and leading % signs from the description
            const descriptionWithoutPercent = description
                .replace(/^%/, "")
                .replace(/%$/, "");
            descriptionText = packageNLS[descriptionWithoutPercent];
        }
        optionsReadmeSection += `| ${option} | ${configurationProperties[option].type} | ${descriptionText} | ${configurationProperties[option].default} | ${configurationProperties[option].enum ? configurationProperties[option].enum.join(", ") : ""} | \n`;
    }

    optionsReadmeSection += "## mssql.shortcuts\n\n";
    optionsReadmeSection += "| Shortcut | Default | \n" + "|--------|------| \n";
    const configurationPropertiesShortcuts = configurationProperties["mssql.shortcuts"].default;
    for (const shortcut of Object.keys(configurationPropertiesShortcuts)) {
        if(shortcut === "_comment") {
            continue;
        }
        optionsReadmeSection += `| ${shortcut} | ${configurationPropertiesShortcuts[shortcut]} | \n`;
    }

    fs.writeFile("./documentation/options.md", optionsReadmeSection);
    done();
});
